...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      if (anyMotion && typeof anyMotion.requestPermission === 'function') {
        ...
        window.addEventListener('deviceorientation', do_);
      }
    };
  ...
  const [paddedIcon, setPaddedIcon] = useState<string | null>(null);

  // Generate a pre-padded version of the icon to force exact spacing in the CSS mask
  useEffect(() => {
    if (!iconUrl) return;
    const img = new Image();
    img.src = iconUrl;
    ...
    img.onload = () => {
      ...
      // 30% size + 20% gap = 50% tile. Icon is 60% of the tile.
      const size = img.width / 0.6; 
      canvas.width = size;
      canvas.height = size;
      const ctx = canvas.getContext('2d');
      if (ctx) {
        ctx.drawImage(img, (size - img.width) / 2, (size - img.height) / 2, img.width, img.height);
        setPaddedIcon(canvas.toDataURL('image/png'));
      }
    };
  }, [iconUrl]);
